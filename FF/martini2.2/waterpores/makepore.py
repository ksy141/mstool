...
def parse_args():
    ...
    parser.add_argument('--height', type=float, default=50.0, help='provide a height of the cylinder in A')
...
def make_structure(radius, height, resname, chain):
    resid   = 1
    data    = {'resname': resname, 'resid': 1, 'chain': chain,
               'name': [], 'x': [], 'y': [], 'z': []}

    Nr = int(2 * np.pi * radius // 5.0)
    theta = 2 * np.pi / Nr
...
def main():
    ...
    make_structure(args.radius, args.height, resname, args.chain)
