...
from .util import completeTetra, completeTetra3
import pandas as pd
import numpy as np

def capTerminiResidues(structure, out=None, cap_termini={}, verbose=True):
    ...
    termatoms = ['H2', 'H3', 'HT2', 'HT3', 'OXT', 'OT2', 'OXT2']
    ...
    for chainnum, chain in enumerate(chains):
        ...
        if Ncap != 'NH3' and Ncap != 'ACE':
            if verbose: print('Available N-terminus: NH3 or ACE... Using NH3')
            Ncap = 'NH3'

        if Ccap != 'COO' and Ccap != 'NMA':
            if verbose: print('Available C-terminus: COO or NMA... Using COO')
            Ccap = 'COO'
        
        bA1     = proteinu.atoms.chain == chain
        resids  = proteinu.atoms[bA1]['resid'].unique()
        Nfirst  = min(resids)
        Clast   = max(resids)
        if Nfirst == Clast:
            Nterms = [Nfirst]
            ...
            diff    = resids[1:] - resids[:-1]
            Nterms  = resids[1:][diff != 1]
            Nterms  = np.concatenate([[Nfirst], Nterms])
            Cterms  = resids[:-1][diff != 1]
            Cterms  = np.concatenate([Cterms, [Clast]])
        
        for Ntermresid, Ctermresid in zip(Nterms, Cterms):
            if verbose: print(f'@N/{chainnum}:{Ntermresid} = {Ncap}')
            if verbose: print(f'@C/{chainnum}:{Ctermresid} = {Ccap}')
            ...
            if Ncap == 'NH3':
                if resn != 'PRO' and len(Hatoms) > 0:
                    ...
                    dr2, dr3, dr4 = completeTetra3(dr1)
                    dr2 *= 1.1
                    dr3 *= 1.1
                    dr4 *= 1.1
                    data['name'].extend(['HN', 'HT2', 'HT3'])
                    data['resname'].extend([resn, resn, resn])
                    data['segname'].extend([segn, segn, segn])
                    data['resid'].extend([Ntermresid, Ntermresid, Ntermresid])
                    data['chain'].extend([chain, chain, chain])
                    data['x'].extend([Nx + dr2[0], Nx + dr3[0], Nx + dr4[0]])
                    data['y'].extend([Ny + dr2[1], Ny + dr3[1], Ny + dr4[1]])
                    data['z'].extend([Nz + dr2[2], Nz + dr3[2], Nz + dr4[2]])
    ...
    protein.sort_values(by=['chain', 'resid'], inplace=True)
