...
def probelize(u, probes, out=None, molfrac=0.1, waterresname='W', probechain='X'):
    bA = u.atoms.resname == waterresname
    nonwateratoms = u.atoms[~bA]
    wateratoms    = u.atoms[bA].sample(frac=1, ignore_index=True) # a new object
    Noct          = int(len(wateratoms) * molfrac)
    ...
    for i, refmol in enumerate(refmolecules):
        atoms = refmol.atoms                       # DataFrame with at least ['resname','name']
        n = len(atoms)
        if n == 0:
            continue

        # repeat the i-th water row n times
        base = pd.concat([wateratoms.iloc[[i]].assign()]*n, ignore_index=True)

        # vectorized field fills
        base.loc[:, 'resname'] = atoms['resname'].to_numpy()
        base.loc[:, 'name']    = atoms['name'].to_numpy()
    ...
    probeatoms = pd.concat(chunks, ignore_index=True)

    # combine
    u.atoms = pd.concat([nonwateratoms, newwateratoms, probeatoms], ignore_index=True)
    u.sort(by=['chain','resid'])
...
def ProbeMartini(structure=None, probes=None, out=None, t=None,
                 dimensions=None,
                 solventdr=4.93, removedr=6.0, waterslab=0.8, waterchain='W', waterresname='W',
                 center=True, molfrac=0.01, pbc=True, probechain='X'):
    ...
    for pp in probes:
        ppu = Universe(pp)
        ppu.atoms[['x','y','z']] -= ppu.atoms[['x','y','z']].mean(axis=0)
        probeMols.append(ppu)

    newu = probelize(solvatedu, probes=probeMols, out=out, molfrac=molfrac, waterresname=waterresname, probechain=probechain)
    if out: newu.write(out)
    return newu
